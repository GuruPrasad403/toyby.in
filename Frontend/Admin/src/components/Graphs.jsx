...
export default function Graphs() {
  ...
  // Fetch the data
  const { data } = useFetch({
    route: "api/reports/most-sold-products",
    method: "GET",
    token:
  ...
  useEffect(() => {
    if (data) {
      setProductData(data.mostSoldProducts);
    }
  }, [data]);
    ...
    <div className="absolute grid grid-cols-8 grid-rows-1 gap-4 overflow-x-hidden py-5 ">
      ...
      <div className="grid grid-flow-row col-span-2 p-4 shadow-2xl rounded-xl grid-cols-1 overflow-y-auto max-h-[550px]">
        <div className="p-5" style={{ position: "sticky" }}>
          <h1 className="text-xl font-semibold underline text-[#]">
        ...
        {productData.map((product, index) => (
          <div
            ...
            onMouseEnter={() => {
              setHoverProdcut(product);
              sethover(true);
            }}
            onMouseLeave={() => {
              setHoverProdcut(null);
              sethover(false);
            }}
          >
            <div>
      ...
      {hover ? (
        <div
          className={` absolute rounded-xl shadow-xl p-5 bg-white w-52`}
          style={{ bottom: mousePosition.y - 273, left: mousePosition.x - 383 }}
        >
          <div>
            <img
              src={hoverProduct.thumbnail}
              alt="this is the HoverProduct"
            />
          </div>
          <div>
            ...
            <h2
              className={`text-md font-semibold ${
                hoverProduct.stock > 5 ? "text-green-500" : "text-red-500"
              }`}
            >
              Stock : {hoverProduct.stock}
            </h2>
